...
| **ISS-034** | XSS almacenado mediante avatar SVG servido como estático             | Backend            | 09/08/2026      | Detectado al escribir las pruebas unitarias de `saveBase64Avatar`: la extensión del archivo se derivaba del tipo declarado en el `data:` URL, controlado por quien sube el avatar, de modo que `image/svg` quedaba almacenado como `.svg`. Los avatares sí se sirven con `express.static`, por lo que el archivo se entregaba con `Content-Type: image/svg+xml` y ejecutaba su script al abrir la URL directa. Se aplicó el criterio de ISS-028: lista blanca de tipos rasterizados con extensión canónica derivada de la tabla, exclusión explícita de SVG y límite de tamaño configurable. |
...
### [ISS-034] XSS almacenado mediante avatar SVG servido como estático

- **Código Afectado (Backend)**: [avatar.ts](file:///c:/Workspace/certvault/backend/src/utils/avatar.ts), [server.ts](file:///c:/Workspace/certvault/backend/src/server.ts) (montaje estático de `/uploads/avatars`).
- **Origen del hallazgo**: Detectado al escribir las pruebas unitarias de `saveBase64Avatar`, que hasta entonces no tenía cobertura alguna.
- **Causa Raíz**: La extensión del archivo almacenado se derivaba del tipo declarado en el `data:` URL (`mime.split('/')[1]`), texto que controla por completo quien sube el avatar. Un `data:image/svg;base64,…` quedaba guardado como `.svg`, y a diferencia de los certificados —que se entregan por endpoint autenticado— los avatares se sirven con `express.static`, que fija el `Content-Type` según la extensión. El resultado era un `image/svg+xml` servido desde el origen de la aplicación: un SVG con script embebido lo ejecuta al abrirse su URL directa. Es el mismo patrón de ISS-028 trasladado a los avatares, con el agravante de la entrega estática. Atenúa el riesgo que los avatares se rendericen dentro de `<img>`, contexto en el que un SVG no ejecuta scripts; hace falta que la víctima abra la URL directa. Tampoco existía límite de tamaño para el contenido recibido.
- **Resolución**:
  - Lista blanca de tipos rasterizados (`png`, `jpeg`, `webp`, `gif`) con su extensión canónica; la extensión ya no proviene del texto del `data:` URL. Se excluye SVG de forma deliberada por ser el único formato de la familia que admite scripts.
  - Se rechaza el contenido vacío y se aplica un límite de tamaño configurable mediante `MAX_AVATAR_FILE_SIZE`, con 2 MB por omisión.
  - **Testing**: `avatar.spec.ts` cubre el rechazo del SVG en sus dos declaraciones (`image/svg` e `image/svg+xml`), los formatos admitidos, el límite de tamaño y la independencia entre el tipo declarado y la extensión resultante.
- **Pendiente**: Los avatares almacenados antes de la corrección conservan su extensión original y se siguen sirviendo como estáticos. Corresponde auditar `uploads/avatars` en busca de archivos `.svg` y equivalentes.
